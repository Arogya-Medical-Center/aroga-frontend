...
export default function BMICalculator() {
  ...
  function interpolateLMS(sex: 'male' | 'female', months: number): LMS | null {
    ...
    const L0 = byMonth[String(lo)], L1 = byMonth[String(hi)];
  ...
  const getChildCategory = () => {
    return 'Requires percentile calculation (refer to CDC/WHO growth charts)';
  };

  const getCategoryColor = (cat: string) => {
    if (cat.includes('Severe') || cat.includes('Class III')) return 'bg-red-50 border-red-300 text-red-800';
    if (cat.includes('Moderate') || cat.includes('Class II')) return 'bg-red-50 border-red-200 text-red-700';
    if (cat.includes('Mild') || cat.includes('Class I')) return 'bg-orange-50 border-orange-200 text-orange-700';
    if (cat.includes('Normal') || cat.includes('Healthy')) return 'bg-green-50 border-green-200 text-green-800';
    if (cat.includes('Overweight') || cat.includes('risk')) return 'bg-orange-50 border-orange-300 text-orange-800';
    if (cat.includes('Underweight')) return 'bg-yellow-50 border-yellow-200 text-yellow-800';
    return 'bg-blue-50 border-blue-200 text-blue-800';
  };

  const getChildPercentileAndCategory = (bmiValue: number, aYears: number, s: Sex) => {
    const months = aYears * 12;
    const lms = s ? interpolateLMS(s, months) : null;
    if (!lms) return { percentile: null as number | null, childCat: 'Enter sex (male/female) to compute BMI-for-age percentile' };

    const { L, M, S } = lms;
    const z = L !== 0 ? (Math.pow(bmiValue / M, L) - 1) / (L * S) : Math.log(bmiValue / M) / S;
    ...
    else if (pct < 95) childCat = 'At risk of overweight (85th–<95th percentile)';
  ...
  const calculateBMI = () => {
    ...
    if (!isNaN(a)) {
      if (a >= 20) {
        ...
        setPercentile(null); // adults don't use percentile
      } else if (a >= 2) {
        const { percentile: pct, childCat } = getChildPercentileAndCategory(bmiValue, a, sex);
        if (pct == null) {
          setCategory(childCat);
          setPercentile(null);
        } else {
          setCategory(childCat);
          setPercentile(pct); // store numeric percentile
        }
      } else {
        setCategory('BMI is not recommended for children under 2 years');
        setPercentile(null);
      }
    } else {
      setCategory(getAdultCategory(bmiValue));
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-green-50 flex items-center justify-center p-4">
      <div className="w-full max-w-4xl">
        ...
        <div className="bg-white rounded-t-2xl shadow-sm p-6 border-b-2 border-blue-500">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Calculate your Body Mass Index using WHO/CDC standards</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-b-2xl shadow-lg p-8">
          <div className="grid md:grid-cols-2 gap-6 mb-6">
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all outline-none"
              >
            ...
            <div className="space-y-2">
              <label htmlFor="weight" className="flex items-center gap-2 text-sm font-semibold text-gray-700">
                <span className="bg-blue-100 text-blue-600 rounded-full w-6 h-6 flex items-center justify-center text-xs">4</span>
                Weight (kg)
              </label>
              <div className="relative">
                <input
          ...
          {/* Results Section */}
          {bmi !== null && (
            <div className={`border-2 rounded-2xl p-6 transition-all ${getCategoryColor(category)}`}>
              <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-4">
                <div className="text-center md:text-left">
                  <h2 className="text-lg font-semibold mb-1">Your BMI Result</h2>
              ...
              <div className="bg-white/50 backdrop-blur-sm rounded-xl p-4">
                ...
                <p className="text-lg font-bold">{category}{percentile != null ? ` • Percentile: ${percentile.toFixed(1)}th` : ''}</p>
              </div>
            </div>
          )}
          ...
          <div className="mt-6 grid md:grid-cols-2 gap-4">
            ...
            <div className="bg-gray-50 rounded-xl p-5">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between bg-white rounded-lg p-2">
                  <span className="font-medium">Severe Thinness</span>
                  <span className="text-gray-600">&lt; 16</span>
                </div>
                <div className="flex justify-between bg-white rounded-lg p-2">
                  <span className="font-medium">Moderate Thinness</span>
                  <span className="text-gray-600">16 - 17</span>
                </div>
                <div className="flex justify-between bg-white rounded-lg p-2">
                  <span className="font-medium">Mild Thinness</span>
                  <span className="text-gray-600">17 - 18.5</span>
                </div>
                <div className="flex justify-between bg-white rounded-lg p-2">
                  <span className="font-medium">Normal</span>
                  <span className="text-gray-600">18.5 - 25</span>
                </div>
                <div className="flex justify-between bg-white rounded-lg p-2">
                ...
                <div className="flex justify-between bg-white rounded-lg p-2">
                  <span className="font-medium">Obese Class I</span>
                  <span className="text-gray-600">30 - 35</span>
                </div>
                <div className="flex justify-between bg-white rounded-lg p-2">
                  <span className="font-medium">Obese Class II</span>
                  <span className="text-gray-600">35 - 40</span>
                </div>
                <div className="flex justify-between bg-white rounded-lg p-2">
                  <span className="font-medium">Obese Class III</span>
                  <span className="text-gray-600">&gt; 40</span>
                </div>
              </div>
              <p className="text-xs text-gray-500 mt-3 italic">WHO recommended classification for adults</p>
            </div>
            ...
            <div className="bg-gray-50 rounded-xl p-5">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between bg-white rounded-lg p-2">
                  <span className="font-medium">Healthy weight</span>
                  <span className="text-gray-600">5th - 85th percentile</span>
                </div>
                <div className="flex justify-between bg-white rounded-lg p-2">
                  <span className="font-medium">At risk of overweight</span>
                  <span className="text-gray-600">85th - 95th percentile</span>
                </div>
                <div className="flex justify-between bg-white rounded-lg p-2">
                  <span className="font-medium">Overweight</span>
              ...
              <p className="text-xs text-gray-500 mt-3 italic">
                CDC recommends BMI-for-age percentiles (sex-specific). Refer to growth charts for accurate classification.
              </p>
              <div className="mt-3 p-3 bg-blue-50 rounded-lg">
                <p className="text-xs text-blue-800 font-medium">
                  ⚠️ Note: For ages 2-20, percentile calculations require sex-specific growth chart data. Consult a healthcare provider for accurate assessment.
                </p>
              </div>
            </div>
          </div>
